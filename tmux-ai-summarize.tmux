...
bind-key -T copy-mode-vi $quoted_key send-keys -X copy-selection-no-clear \; send-keys -X copy-selection-and-cancel "ai-summarize-#{pane_id}-" \; run-shell "TMUX_AI_SUMMARIZE_CLIENT=#{q:client_name} TMUX_AI_SUMMARIZE_BUFFER_SCOPE=#{q:pane_id} $quoted_runner_path"
EOF
